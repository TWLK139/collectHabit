<template>
	<view>
		<view class="console">
			<view class="console_for" v-for="(d, i) in browse_data" :key="i">
				<view class="console_view">{{ d.time1.substr(4) }}</view>
				<view class="console_view">{{ d.time2.substr(4) }}</view>
				<view class="console_view">{{ d.x.toFixed(4) }}</view>
				<view class="console_view">{{ d.y.toFixed(4) }}</view>
			</view>
		</view>
		<view id="getBrowse" @touchmove="handletouchmove" @touchstart="handletouchstart" @touchend="handletouchend">
			<view id="getBrowse_title">请滑动这个方框到底部</view>
			<view>
				党章·总纲
				<br>
...
<script>
	export default {
		...
		methods: {
			...
			handletouchend(e) {
				let tem = {};
				tem.time1 = this.start.time + '';
				tem.time2 = (new Date()).valueOf() + '';
			...
			submit() {
				this.stopHold();
				this.submit_data('browse', this.browse_data, this.submit_hold());
			},
			getHold() {
			...
			submit_hold() {
				...
				this.submit_data('hold_browse', all);
			}
		}
	}
</script>
